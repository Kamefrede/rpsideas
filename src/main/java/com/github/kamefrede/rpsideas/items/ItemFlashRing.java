...
public class ItemFlashRing extends Item implements ISpellContainer {
    public ItemFlashRing() {
        ...
        addPropertyOverride(new ResourceLocation(Reference.MODID, "active"), (stack, world, ent) -> containsSpell(stack) ? 1f : 0f);
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if(!player.isSneaking() && containsSpell(held)) {
            ...
            if(world.isRemote) {
                player.openGui(Psiam.INSTANCE, GuiHandler.GUI_FLASH_RING, world, 0, 0, 0);
